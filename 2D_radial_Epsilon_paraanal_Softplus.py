...
Lh=np.float64(0.61)  # Horizontal span of the domain, in meters； This would be the radial coordinate!!
Lrw=np.float64(0.01)  # Radius of drainage pipe. Effective span = [Lh-Lrw,Lh]
Lrs=np.float64(0.31)  # Radius of smear zone. Normally 4-5 times of Lrw
Lv=np.float64(10)
Lt=0.05 # Maximum time span computed, in years
spatial_reso=np.array([0.02,0.5]) # resolution in spatial coordinates, meters
numh=int((Lh-Lrw)/spatial_reso[0])
print(numh)
numv=int(Lv/spatial_reso[1])
cpth=np.arange(Lrw,Lh+0.000001,spatial_reso[0]) # list of collocation points in x-direction.
cptv=np.arange(0,Lv+0.000001,spatial_reso[1]) # list of collocation points in z-direction.
...
cv0=10**(-10)*3600*24*365*8000/9.8 # Undisturbed coef of consolidation; at the boundary of smear zone
cvsred=3 # Disturbed, reduced by a factor of (5), such that cvs=cv0/cvsred.
...
alp=0.5 # such that cv=cv0*(1+alp*z/Lv)
coeft=np.array([hc,h1,alp,h2,0,0])*cv0 # Invert z-axis
...
fluc_o = 1 * np.array(Lmat).T @ rawSlist  # Fluctuation field.
...
Olist=np.exp(np.reshape(matverf.transpose(1,0),-1))+trend # Together, this forms the ground truth for the permeability field. Validated.
dx=spatial_reso[0]
dz=spatial_reso[1]
...
def singleplot3D(matrix,color,title): # Function to plot single field in 3D surface
    ...
    plt.colorbar(surf,ax=ax,shrink=0.5,aspect=20,label=title)
...
def triplot2D(data1,data2,titles,mode,cmap1=custom_colormap,cmap2='bwr',vmin12=None,vmax12=None,vmin3=None,vmax3=None,figsize=(15,5),dpi=100): # Function to plot comparison of 2 fields in 2D contour plane
    ...
    xtlabel=np.array([0.01,0.16,0.31,0.46,0.61])
    ...
    #axes[0].set_title(titles[0], fontsize=16, pad=10)
    axes[0].set_xlabel('Radius r (m)', fontsize=16)
    axes[0].set_ylabel('Depth z (m)', fontsize=16)
    ...
    im2 = axes[1].imshow(data2,
                         ...
                         aspect='auto',
                         interpolation='bicubic')  # Higher-reso
    axes[1].set_title(titles[1], fontsize=16, pad=10)
    axes[1].set_xlabel('r(m)', fontsize=16)
    axes[1].set_ylabel('z(m)', fontsize=16)
    if vmin3 is None:
    ...
    axes[2].set_xlabel('Radius r(m)', fontsize=16)
    axes[2].set_ylabel('Depth z(m)', fontsize=16)
    # Modify tick values
    for ax in axes:
    ...
    cbar12.set_label('Coefficient of consolidation ($m^2$/year)', rotation=90, labelpad=7, fontsize=16)
...
singleplot3D(vec2mat2(Olist),'viridis','Coefficient of consolidation (m^2/year)') # Validated.
...
def basee(vector): # Basis function used as estimators/neurons.
    ...
    b=np.array([Lh,Lv])
    x,z=a/b # Normalized to [0,1]
    lb=np.array([1,x,z,x**2,z**2,x*z,1/(1+a[0]),a[0]/(1+a[0]*Lh)])  # Try adding 1/(1+r)
...
def baseedx(vector): # Basis function's partial derivative field wrt x-axis.
    ...
    b=np.array([Lh,Lv])
    x,z=a/b # Normalized to [0,1]
    lb=np.array([0,1,0,x*2,0,z,-Lh/(1+a[0])**2,Lh/(1+Lh*a[0])**2])/(Lh)
...
def baseedz(vector): # Basis function's partial derivative field wrt z axis.
    ...
    b=np.array([Lh,Lv])
    x,z=a/b # Normalized to [0,1]
    lb = np.array([0, 0, 1, 0, z*2, x,0,0])/Lv
...
#ukmat=[[5,25]]
resize_f=[1,2.5]  # resize factor for locating the measurements. Default reso=0.05,0.2.
numchk=3 # Number of measurements of permeability ch.
#chkmat=np.random.randint([0],[[numv],[numh]],size=(2,numchk)).T # Distribution of ch measurements. z before x.
#chkmat=[[13,21],[24,6],[6,48]]
chkmat=[[20,8],[3,2],[2,11]]
...
for i in chkmat:
    ...
    i[1]=int((i[1]-1)*resize_f[1]+1)
    oplt[i[0],i[1]] = 30
ukt=np.arange(0,int(0.005/dt),20) # List of measurement temporal location. The tunable number is in years; outputs a list
...
def inverse_solver(numt,u0,Rcv,top,bot,left,right,ukmat,chkmat,uk,chk,lam,lr,ltol,gtol,itol,wlst,lamu):
    ...
    #coefe=np.concatenate([wlst+(np.random.randn(len(basese[0])))*0.5],axis=None)
    #last=[3.30024723,-0.10471361,0.18806105,0.69789744,-2.20779547,-12.56892604,-1.06291004,-2.37592096,0.32306212,0.19694973]
    #coefe=last+(np.random.randn(len(basese[0])))*0.1
    ...
    for j in range(itol): # Outer loop controlling the training process.
        ...
        coefe-=lr*update # So fixed because the gradients are too small, might be an error.
        if loss_t<=ltol or np.sum(update**2)<=gtol:
            print('Early-stopped at epoch ',j)
            break
    # End of loop j
    np.save('QAPILM_rad_eps_iso_loss.npy',losses)
...
def func_inv_sol(chm,rcv,ilim,u0,bcs,ukmat,chkmat,ukt,lam,lr,ltol,gtol,itol,lamu):
    ...
    wlst, res, rank, s = np.linalg.lstsq(basese, isoftplus(trend + np.exp(fluc)), rcond=None)  # Ideal possible solution via LSM
    ...
    #triplot2D(u1[int(0.003/dt)-1], ue[int(0.003/dt)-1], np.array(['Real u/u_0 at t=1 day', 'Predicted u/u_0 at t=1 day', 'Error']),0,'jet','bwr',0,1,-0.2,0.2)
...
def create_dataset(chm,rcv,ilim,u0,bcs,ukmat,chkmat,ukt,lam,lr,ltol,gtol,itol,lamu,filename_prefix='consolidationRad'):
    ...
    for i in range(len(chkmat)):
        point=chkmat[i]
        x0=point[1]*spatial_reso[0]+Lrw
        y0=point[0]*spatial_reso[1]
        c_arr[i,0]=x0
        c_arr[i,1]=y0
